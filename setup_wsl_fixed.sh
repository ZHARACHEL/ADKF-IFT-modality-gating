...
# 步骤 2: 安装必要工具
echo ""
echo "[2/7] 安装必要工具 (wget, git, unzip, build-essential)..."
...
# 步骤 5: 初始化 Git 子模块（如果是 Git 仓库）
echo ""
echo "[5/7] 检查并初始化 Git 子模块..."
if [ -d ".git" ]; then
    git submodule update --init --recursive
    echo "Git 子模块初始化完成"
else
    echo "⚠️  警告：这不是一个 Git 仓库，跳过子模块初始化"
    echo "   如果需要 PAR 子模块，请手动下载或使用 git clone 获取完整仓库"
fi

# 步骤 6: 创建 Conda 环境
echo ""
echo "[6/7] 创建 Conda 环境 (这可能需要 10-20 分钟)..."
...
# 步骤 7: 下载数据集
echo ""
echo "[7/7] 下载 FS-Mol 数据集 (约 1.5 GB)..."
